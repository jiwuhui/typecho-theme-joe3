...
/**
 * 文章底部操作（标签 + 分享 + 打赏）（移植自 Halo 版 post_operate.html）
 */
$joeShareUrl = joe_site_url() . $this->path;
$joeShareTitle = (string) $this->title;
$joeShareAuthor = (string) $this->author->screenName;
$joeShareCover = joe_post_cover($this);
$joeShareDesc = joe_excerpt($this, 100);
$joeShareTpl = (string) joe_opt('share_link_template');
if ($joeShareTpl === '') {
    $joeShareTpl = '{postUrl}';
}
$joeShareTpl = str_replace(
    ['{postUrl}', '{postTitle}', '{postAuthor}', '{postDescription}', '{BlogTitle}', '{BlogUrl}'],
    [$joeShareUrl, $joeShareTitle, $joeShareAuthor, $joeShareDesc, joe_site_title(), joe_site_url()],
    $joeShareTpl
);
?>
<div class="joe_detail__operate">
    <div class="joe_detail__operate-tags">
        <?php $this->tags('<a href="{permalink}" >{name}</a>', true, ''); ?>
    </div>
            ...
            <div class="share-icon-list">
                <?php if (joe_is_on('enable_share_link')): ?>
                    <a class="icon-share-link" href="javascript:;" rel="noopener noreferrer" title="复制文章链接"
                       data-post-title="<?php echo htmlspecialchars($joeShareTitle); ?>"
                       data-post-description="<?php echo htmlspecialchars($joeShareDesc); ?>"
                       data-template="<?php echo htmlspecialchars($joeShareTpl); ?>"
                       data-post-author="<?php echo htmlspecialchars($joeShareAuthor); ?>">
                ...
                <?php if (joe_is_on('enable_share_weibo')): ?>
                    <a href="http://service.weibo.com/share/share.php?sharesource=weibo&title=分享：『<?php echo urlencode($joeShareTitle); ?>』，作者：<?php echo urlencode($joeShareAuthor); ?>，原文链接：<?php echo urlencode($joeShareUrl); ?>&pic=<?php echo urlencode($joeShareCover); ?>"
                ...
                <?php if (joe_is_on('enable_share_qzone')): ?>
                    <a href="https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=<?php echo urlencode($joeShareUrl); ?>&sharesource=qzone&title=『<?php echo urlencode($joeShareTitle); ?>』，作者：<?php echo urlencode($joeShareAuthor); ?>&pics=<?php echo urlencode($joeShareCover); ?>"
                ...
                <?php if (joe_is_on('enable_share_qq')): ?>
                    <a href="https://connect.qq.com/widget/shareqq/index.html?url=<?php echo urlencode($joeShareUrl); ?>&title=『<?php echo urlencode($joeShareTitle); ?>』，作者：<?php echo urlencode($joeShareAuthor); ?>&pics=<?php echo urlencode($joeShareCover); ?>"
