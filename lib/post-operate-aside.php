...
/**
 * 文章侧边悬浮操作（点赞/评论/分享/打赏）（移植自 Halo 版 post_operate_aside.html）
 */
$joeAsideShareUrl = joe_site_url() . $this->path;
$joeAsideShareTitle = (string) $this->title;
$joeAsideShareAuthor = (string) $this->author->screenName;
$joeAsideShareCover = joe_post_cover($this);
$joeAsideCommentNum = (int) $this->commentsNum;
$joeAsideUseWaline = joe_opt('comment_option') === 'waline' && trim((string) joe_opt('waline_serverURL')) !== '';
?>
<ul class="aside_operations">
    <?php if (joe_is_on('enable_like')): ?>
        <?php global $JOE_FAVORITE_MODE;
        $JOE_FAVORITE_MODE = 'aside'; ?>
    ...
    <?php if (!joe_is_on('enable_clean_mode') && joe_is_on('enable_comment')): ?>
        <li class="post-operate-comment">
            ...
            <?php if ($joeAsideCommentNum > 0 && !$joeAsideUseWaline): ?>
                <span class="visible"><?php echo $joeAsideCommentNum; ?></span>
            <?php elseif ($joeAsideUseWaline): ?>
                <span class="visible waline-comment-count"
                      data-path="<?php echo joe_site_url() . $this->path; ?>">0</span>
            <?php endif; ?>
        </li>
    <?php endif; ?>
    <?php if (joe_is_on('enable_share')): ?>
        <li class="post-operate-share">
            <i class="joe-font joe-icon-huifu"></i>
            <div class="share-icon-list">
                <?php if (joe_is_on('enable_share_weixin')): ?>
                ...
                <?php endif; ?>
                <?php if (joe_is_on('enable_share_weibo')): ?>
                    <a href="http://service.weibo.com/share/share.php?sharesource=weibo&title=分享：『<?php echo urlencode($joeAsideShareTitle); ?>』，作者：<?php echo urlencode($joeAsideShareAuthor); ?>，原文链接：<?php echo urlencode($joeAsideShareUrl); ?>&pic=<?php echo urlencode($joeAsideShareCover); ?>"
                ...
                <?php endif; ?>
                <?php if (joe_is_on('enable_share_qq')): ?>
                    <a href="https://connect.qq.com/widget/shareqq/index.html?url=<?php echo urlencode($joeAsideShareUrl); ?>&title=『<?php echo urlencode($joeAsideShareTitle); ?>』，作者：<?php echo urlencode($joeAsideShareAuthor); ?>&pics=<?php echo urlencode($joeAsideShareCover); ?>"
                ...
                <?php endif; ?>
                <?php if (joe_is_on('enable_share_qzone')): ?>
                    <a href="https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=<?php echo urlencode($joeAsideShareUrl); ?>&sharesource=qzone&title=『<?php echo urlencode($joeAsideShareTitle); ?>』，作者：<?php echo urlencode($joeAsideShareAuthor); ?>&pics=<?php echo urlencode($joeAsideShareCover); ?>"
                ...
                <?php endif; ?>
                <?php if (joe_is_on('enable_share_link')): ?>
                    <a class="icon-share-link" href="javascript:;" rel="noopener noreferrer" title="复制文章链接"
                       data-post-title="<?php echo htmlspecialchars($joeAsideShareTitle); ?>"
                       data-post-description="<?php echo htmlspecialchars(joe_excerpt($this, 100)); ?>"
                       data-template="<?php echo htmlspecialchars((string) joe_opt('share_link_template') ?: '{postUrl}'); ?>"
                       data-post-author="<?php echo htmlspecialchars($joeAsideShareAuthor); ?>">
                        <svg class="icon" viewBox="0 0 1024 1024" version="1.1"
                             xmlns="http://www.w3.org/2000/svg" p-id="3509" width="30" height="30">
                            <path
